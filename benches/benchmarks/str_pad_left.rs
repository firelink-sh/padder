...
pub fn str_pad_100_left(c: &mut Criterion) {
    ...
    c.bench_function("format! 100 la", |b| {
        b.iter(|| black_box(format!("{:<width$}", "praise the sun!")))
    });
...
pub fn str_pad_1000_left(c: &mut Criterion) {
    ...
    c.bench_function("format! 1000 la", |b| {
        b.iter(|| black_box(format!("{:<width$}", "solaire is awesome")))
    });
...
pub fn str_pad_10000_left(c: &mut Criterion) {
    ...
    c.bench_function("format! 10000 la", |b| {
        b.iter(|| black_box(format!("{:<width$}", "don't you dare go hollow..!!#\"")))
    });

}

pub fn str_pad_100000_left(c: &mut Criterion) {
    let width: usize = 100000;
    c.bench_function("str pad 100000 la", |b| {
        b.iter(|| black_box("東風 ぬが ㅀㆈ".pad(width, Alignment::Left, 'µ')))
    });
    c.bench_function("format! 100000 la", |b| {
        b.iter(|| black_box(format!("{:<width$}", "東風 ぬが ㅀㆈ")))
    });
}
